...
describe('file: create-report/src-files', () => {
  describe('function: parseSrcFiles', () => {
    ...
    it('Throws an error if it is not a valid glob', () => {
      ...
      expect(() => readSrcFiles(breakingSrcFiles)).toThrow(`srcFiles isn't a valid glob pattern.`);
    });

    it('Throws an error if it does not find any file', () => {
      const breakingSrcFiles = path.resolve(__dirname, '../fixtures/src-files/**/*.txt');
      expect(() => readSrcFiles(breakingSrcFiles)).toThrow('srcFiles glob has no files.');
    });
  });
});
