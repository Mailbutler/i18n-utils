(function (global, factory) {
  ...
  function resolveConfig() {
    ...
    try {
      const pathToConfigFile = path__default["default"].resolve(
        process.cwd(),
        "./vue-i18n-extract.config.js"
  ...
  function extractI18NItemsFromSrcFiles(sourceFiles) {
  ...
  function parseSrcFiles(srcFiles) {
  ...
  function readLanguageFiles(src) {
    ...
    return targetFiles.map((f) => {
      ...
      if (isJSON) {
        langObj = JSON.parse(
          fs__default["default"].readFileSync(langPath, "utf8")
        );
      } else if (isYAML) {
        langObj = yaml__default["default"].load(
          fs__default["default"].readFileSync(langPath, "utf8")
        );
      } else {
  ...
  function extractI18NLanguageFromLanguageFiles(
    languageFiles,
    dot = Dot__default["default"]
  ) {
  ...
  function writeMissingToLanguageFiles(
    parsedLanguageFiles,
    missingKeys,
    dot = Dot__default["default"]
  ) {
  ...
  function removeUnusedFromLanguageFiles(
    parsedLanguageFiles,
    unusedKeys,
    dot = Dot__default["default"]
  ) {
  ...
  function parselanguageFiles(languageFiles, dot = Dot__default["default"]) {
    return extractI18NLanguageFromLanguageFiles(
  ...
  function mightBeDynamic(item) {
    return (
      item.path.includes("${") &&
      !!item.previousCharacter.match(/`/g) &&
      !!item.nextCharacter.match(/`/g)
    );
  ...
  function extractI18NReport(vueItems, languageFiles) {
    const missingKeys = [];
    const unusedKeys = [];
    const maybeDynamicKeys = vueItems
    ...
    Object.keys(languageFiles).forEach((language) => {
      ...
      const missingKeysInLanguage = vueItems
        ...
        .filter(
          (vueItem) =>
            !languageItems.some(
              (languageItem) => vueItem.path === languageItem.path
            )
        )
        .map((vueItem) =>
          _extends({}, stripBounding(vueItem), {
            language,
          })
        );
  ...
  async function createI18NReport(options) {
    ...
    if (!srcFilesGlob)
      throw new Error("Required configuration srcFiles is missing.");
    if (!languageFilesGlob)
      throw new Error("Required configuration languageFiles is missing.");
    const dot =
      typeof separator === "string"
        ? new Dot__default["default"](separator)
        : Dot__default["default"];
    const srcFiles = readSrcFiles(
      path__default["default"].resolve(process.cwd(), srcFilesGlob)
    );
    const languageFiles = readLanguageFiles(
      path__default["default"].resolve(process.cwd(), languageFilesGlob)
    );
    const I18NItems = extractI18NItemsFromSrcFiles(srcFiles);
    const I18NLanguage = extractI18NLanguageFromLanguageFiles(
      languageFiles,
      dot
    );
    ...
    if (output) {
      await writeReportToFile(
        report,
        path__default["default"].resolve(process.cwd(), output)
      );
  ...
  exports.extractI18NItemsFromSrcFiles = extractI18NItemsFromSrcFiles;
  exports.extractI18NLanguageFromLanguageFiles =
  ...
  exports.readSrcFiles = readSrcFiles;
